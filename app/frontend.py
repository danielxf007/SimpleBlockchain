...
from util.printing import print_messages, print_tx_inputs, print_tx_input_index_utxo_value, print_block, print_utxos
import argparse
...
class Frontend:
    ...
    def create_wallet_commands(self):
        ...
        self.wallet_commands.add_argument('--download_lock_scripts', type=str,
        ...
        self.wallet_commands.add_argument('--create_tx', action='store_true',
                    help='Makes a transaction using the current tx inputs and utxos from the wallet')
        ...
        while True:
            try:
                if self.app_state == "main_menu":
                    args = self.parser.parse_args(input('command> ').split())
                    print()
                    if args.wallet_user_name:
                    ...
                    if args.get_keys:
                        ...
                        print_utxos(self.backend.get_created_utxos())
                    elif args.fee:
                        print(self.backend.get_fee())
                    elif args.create_tx:
                        print(self.backend.create_tx())
                    elif args.log_out:
                        self.app_state = "main_menu"
